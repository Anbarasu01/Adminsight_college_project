...
const PublicPortal = () => {
  ...
  const mockNotifications = [
    { id: 1, type: "update", message: "Your complaint #TRK123 has been assigned to Revenue Department", time: "2 hours ago", read: false },
    { id: 2, type: "resolved", message: "Complaint #TRK456 has been resolved successfully", time: "1 day ago", read: true },
    { id: 3, type: "info", message: "New service announcement: Faster response times implemented", time: "2 days ago", read: true },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const generatedId = `TRK${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    ...
    try {
      ...
      setTimeout(() => {
        const mockStatusData = {
          ...
          updates: [
            { date: new Date().toISOString(), message: "Complaint received and logged" },
            { date: new Date().toISOString(), message: "Assigned to field officer for inspection" },
            { date: new Date().toISOString(), message: "Work in progress - estimated completion in 3 days" }
          ]
        };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <Link to="/" className="flex items-center space-x-3">
              ...
              <span className="text-xl font-bold text-gray-900">ADMINSIGHT</span>
            </Link>
        ...
        {activeTab === "report" && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { value: "85%", label: "Problems Resolved", icon: "✅", color: "green" },
                { value: "2.4h", label: "Avg Response Time", icon: "⚡", color: "blue" },
                { value: "10+", label: "Departments", icon: "🏢", color: "purple" },
              ].map((stat, index) => (
            ...
            <div className="bg-white rounded-2xl shadow-lg border">
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-8 py-6 rounded-t-2xl">
                <h2 className="text-2xl font-bold text-white">Problem Report Form</h2>
                <p className="text-blue-100">Fill in the details below to report your issue</p>
              </div>
                ...
                {submitStatus === "success" && (
                  <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-semibold text-green-800">Problem reported successfully!</p>
                        <p className="text-green-600 text-sm">
                          Tracking ID: <strong>{trackingId}</strong> - Save this for status checking
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex justify-between items-center pt-6 border-t">
                  <button
                    ...
                    onClick={() => setFormData({
                      name: "", email: "", phone: "",
                      department: "Revenue & Disaster Management",
                      problemTitle: "", description: "", location: "",
                    })}
              ...
              {statusData && (
                <div className="mt-8 p-6 bg-gray-50 rounded-lg border">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold">Status for: {statusData.trackingId}</h3>
                    <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(statusData.status)}`}>
                      {statusData.status}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{new Date(statusData.lastUpdate).toLocaleDateString()}</p>
                    </div>
                  </div>
